...
export function createSuggestedPlan({ goals, habits, categories, workSchedule, date = new Date().toISOString() }) {
  ...
  if (!blocks.length && pockets[0]) {
    const fallbackDuration = Math.min(60, pockets[0].end - pockets[0].start);
    blocks.push({
      id: "fallback-focus-block",
      title: "Intentional focus block",
      categoryId: categories[0]?.id || "general",
      categoryLabel: categories[0]?.label || "Personal",
      source: "fallback",
      status: "suggested",
      start: fromMinutes(pockets[0].start),
      end: fromMinutes(pockets[0].start + fallbackDuration),
      notes: "Use this time for the one thing that best matches your mission today.",
      reminderOffsetMinutes: 15
    });
  }
